...
test_that("refsets refer to original", {
  ...
  expect_identical(ss$b, 6:8)
  dfr$b <- 1:5
  expect_identical(ss$b, 1:3)
})
...
test_that("refsets change original", {
  ...
  refset(ss, dfr, 1:3, 1:2)
  ss$b <- 1:3
  expect_identical(dfr$b, c(1:3,9:10))
})
...
test_that("subsets of refsets work and change original", {
  ...
  dfr$a <- 6:10
  expect_identical(ss[1:2,1], 8:9)
  expect_identical(ss[[3,1]], 10L)
  expect_identical(ss[c(TRUE,TRUE,FALSE),], dfr[3:4,])
  ss$a <- 1:3
})
...
test_that("refsets of refsets work", {
  ...
  refset(ss, dfr, 3:5, 1:2)
  refset(sss, ss, 1:2, 1)
  expect_identical(sss, 3:4)
  dfr$a <- 6:10
  expect_identical(sss, 8:9)
  ss$a <- 1:3
  expect_identical(sss, 1:2)
})
...
test_that("refsets of refsets change original", {
  ...
  refset(ss, dfr, 3:5, 1:2)
  refset(sss, ss, 1:2, 1)
  expect_identical(sss, dfr$a[3:4])
  sss <- 8:9
  expect_identical(dfr$a[3:4], 8:9)
  expect_identical(ss$a[1:2], 8:9)
})
...
test_that("reassignment breaks link", {
  ...
  dfr$b <- 1:5
  expect_identical(ss$b,  1:3)
  expect_identical(copyss$b, 6:8)
})

test_that("name-based indexing works", {
  dfr <- data.frame(a=1:5, b=6:10)  
  refset(ss, dfr, "a")
  expect_identical(ss$a[1:3], 1:3)
  dfr$a <- 5:1
  expect_identical(ss$a[1:3], 5:3)
  ss$a <- 1:5
  expect_identical(dfr$a, 1:5)
})
...
test_that("dynamic indexing works", {
  ...
  expect_identical(nrow(ss), 3L)
  dfr$a <- 2:6
  expect_identical(nrow(ss), 2L)
  ss$a <- 3:4
  expect_identical(nrow(ss),1L)
  expect_identical(nrow(ss.static), 3L)
  expect_identical(ss.static$a, c(3L, 4L, 4L))
})
...
test_that("assigning into an environment works", {
  ...
  expect_identical(e2$ss$a, 1:3)
  e2$dfr$a <- 6:10
  expect_identical(e2$ss$a, 6:8)
  e2$ss$a <- 1:3
  expect_identical(e2$dfr$a, c(1:3, 9:10))
})
...
test_that("empty arguments work", {
  ...
  expect_identical(dim(ss), c(3L, 3L))
  expect_identical(dim(ss2), c(5L, 2L))
  
  dfr <- rbind(dfr, rep(10,3))
  expect_identical(dim(ss2), c(6L, 2L))
  expect_identical(dim(ss2.static), c(5L, 2L))
})
...
test_that("drop works", {
  ...
  expect_null(dim(ssd))
  expect_identical(dim(ssnd), c(5L, 1L)) 
...
test_that("vectors work", {
  vec <- 1:5
  refset(ssv, vec, 1:2)
  expect_equal(ssv, 1:2)
  vec[2:3] <- 1
  expect_equal(ssv, c(1,1))
  ssv <- 4:5
  expect_equal(vec, c(4:5, 1, 4:5))
})
...
test_that("views work in all environments", {
  g <- function() {
    ...
    expect_identical(ss$b, 1:3)
    ss$a <- 6:8 
    expect_identical(dfr$a, c(6:8, 4:5))
  } 
  g()
})
...
test_that("two-argument form works", {
  ...
  refset(ss, dfr[1,1])
  expect_identical(ss, 1L)
  refset(ss2, dfr[c(FALSE, TRUE), "b"])
  expect_identical(ss2, c(6L, 8L))
  dfr$a <- 4:1
  dfr$b <- 8:5
  expect_identical(ss, 4L)
  expect_identical(ss2, c(7L, 5L))
  ss <- 10L
  expect_identical(dfr[1,1], 10L)
  ss2 <- c(NA, NA)
  expect_identical(dfr$b, c(8L, NA, 6L, NA))
...
test_that("two-argument form with [[ works", {
  dfr <- data.frame(a=1:4, b=5:8)
  refset(ss, dfr[["a"]])
  expect_identical(ss, 1:4)
  dfr$a <- 4:1
  expect_identical(ss, 4:1)
  ss <- 1:4
  expect_identical(dfr$a, 1:4)
})
...
test_that("complex two-argument forms work", {
  ...
  refset(ss, dfr$b[2:3])
  expect_identical(ss, 6:7)
  dfr$b <- LETTERS[1:4]
  expect_identical(ss, LETTERS[2:3])
  ss <- LETTERS[25:26]
  expect_identical(dfr$b, c("A", "Y", "Z", "D"))
  
  dfr <- data.frame(a=1:4, b=5:8, c=9:12)
  refset(ss2, dfr[c(TRUE, FALSE), c("a", "c")])
  expect_identical(ss2, dfr[c(1,3), c("a", "c")])
  dfr$a <- 4:1
  expect_identical(ss2, dfr[c(1,3), c("a", "c")])
  ss2 <- 10
  expect_identical(dfr$a, c(10, 3, 10, 1))
})

test_that("changing nrow of original object works", {
  dfr <- data.frame(a=1:8, b=1:8)
  refset(ss, dfr[1:(nrow(dfr)/2),])
  expect_identical(nrow(ss), 4L)
  dfr <- dfr[1:4,]
  expect_identical(nrow(ss), 2L)
  dfr <- rbind(dfr, dfr, dfr)
  expect_identical(nrow(ss), 6L)
})

test_that("changing ncol of original object works", {
  mx <- matrix(1:16, ncol=4)
  refset(ss, mx[, c(FALSE, TRUE)])
  expect_identical(ncol(ss), 2L)
  mx <- cbind(mx, mx)
  expect_identical(ncol(ss), 4L)
})
